...
// 전국 지도. 경계·점수는 클라이언트가 /api/geojson · /api/mapdata 로 로드.
// 서버는 인트로 KPI 하이라이트만 계산해 전달(경량).
export default function HomePage() {
  const districts = loadDistricts();
  const scores = loadScores();

  const rows = districts.features.map((f) => {
  ...
  const gentriRow = [...rows]
    .filter((r) => r.s.gentriFlag)
    .sort((a, b) => b.s.gentriStage - a.s.gentriStage)[0];

  // 급등 핫스팟 (모멘텀 상위 8) — 핫스팟 투어용 좌표
