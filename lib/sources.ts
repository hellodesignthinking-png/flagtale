...
export const SOURCES: DataSource[] = [
  ...
  },
  {
    ...
    keyEnv: ["DATA_GO_KR_KEY"],
    cadence: "연(census기반)",
    note: "✅ 연동됨(활용신청 승인): 시군구별 쇠퇴진단 등급 1~10, 인구·경제·물리 3부문 6지표(인구변화·재정자립·사업체증감·지가·공가율·접도율) 평균 = 발전가능성. 41개 지표 中 균형 6종. 잠재력·활성화 별도 서비스는 엔드포인트 미공개로 쇠퇴진단 전부문으로 대체.",
  },
  {
    id: "specialstreet",
    metric: "지역특화거리(문화·상권 거리)",
    axis: "D4 인식·감성 / 문화향유",
    source: "공공데이터포털 전국지역특화거리표준데이터(지자체)",
    url: "https://www.data.go.kr/data/15017322/standard.do",
    keyEnv: ["DATA_GO_KR_KEY"],
    cadence: "수시(표준데이터)",
    note: "✅ 연동됨(활용신청 승인): 전국 273개 특화거리(위경도·점포수·지정연도) → 행정동 PIP 귀속(244개→202동). 이태원세계음식거리·경리단길·한남동카페거리 등. 문화영향평가 표현·참여 반영.",
  },
  {
    id: "nabis",
    metric: "지역발전·혁신·창조잠재력지수(시도)",
    axis: "발전가능성 / 창의성",
    source: "NABIS 국가균형발전종합정보시스템 · 산업연구원",
    url: "https://www.nabis.go.kr/",
    keyEnv: [],
    keyless: true,
    cadence: "연(2023 기준)",
    note: "✅ 연동됨(수동 다운로드): 17개 시도 발전·혁신·창조잠재력 지수(2023). 동별 broadcast(시도 컨텍스트). 창조잠재력 = 문화영향평가 '창의성' 공식 반영(서울 0.59>대전 0.50>대구 0.21).",
  },
  {
    id: "culture-facility",
    metric: "문화기반시설 총람(도서관·박물관·미술관·문예회관)",
    axis: "문화향유 / 문화기본권",
    source: "한국문화정보원 전국문화기반시설총람 조회서비스(data.go.kr 15125097)",
    url: "https://www.data.go.kr/data/15125097/openapi.do",
    keyEnv: ["DATA_GO_KR_KEY"],
    cadence: "연(2025 발간)",
    note: "⏳ 활용신청 승인됨 — Swagger 명세 확인 후 커넥터 연동 예정. 현재 venues(네이버 추정)를 이 공식 시설 데이터로 대체할 계획.",
  },
  {
    id: "landprice",
    metric: "개별공시지가 (㎡당)",
